...
async def on_raw_reaction_remove(payload):
	...
	await logs.send(f"https://discord.com/channels/1526558661810327684/{payload.channel_id}\/{(await (await fetch_guild(payload.guild_id)).fetch(payload.user_id)).display_name} removed react: {str(payload.emoji)}")
@client.event
